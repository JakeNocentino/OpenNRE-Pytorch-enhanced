...
//double* majority2;
bool alreadySetupTraining = false;
void CRFModel::InitTrain(Config* conf, DataSet* train_data)
{
	...
	//PyStateFactors = new double[num_feature];
...
void CRFModel::SetupFactorGraphs()
{
	...
	for (int s = 0; s < num_sample; s++)
	{
		...
		sample_factor_graph[s].mapping = this->mapping;
...
{    
	...
	do {
		...
		//dump gradient norms to text file
		//FILE* fp = fopen("gnorms.txt","a");
		//fprintf(fp,"%f\n",g_norm);
		//fclose(fp);
		if (g_norm > 1e-8)
		{
...
void SetPriors(void* model, double* h, double* w, double* l, int G, int labels){
	...
	int N = sample->num_node;
	int Y = crfmodel->num_label;
	for(int n = 0; n < N; n++){
		double* temp = new double[Y];
		double softmax_sum = 0.0;
		bool use_labels = labels && (sample->node[n]->label_type == Enum::KNOWN_LABEL);
		for(int y = 0; y < Y; y++){
			if(use_labels){
				(crfmodel->sample_factor_graph[0]).SetVariableStateFactor(n,y,0.0+(y==sample->node[n]->label));
			}
			else{
				double acc = 0.0;
				for(int g = 0; g < G; g++){
		...
		if(!use_labels){
			for(int y = 0; y < Y; y++){
				(crfmodel->sample_factor_graph[0]).SetVariableStateFactor(n,y,0.0+exp(temp[y])/softmax_sum);
			}
		}
	}
}
...
void Gradient(void* model, double* hgradient, double* h, double* wgradient, double* w, double* l, double* l2, int G){
	CRFModel* crfmodel = (CRFModel*)model;
	DataSample* sample = crfmodel->train_data->sample[0];
	FactorGraph* factor_graph = &(crfmodel->sample_factor_graph[0]);
	int N = sample->num_node;
	int Y = crfmodel->num_label;
...
{   
	...
	factor_graph->labeled_given = true;
	factor_graph->ClearDataForSumProduct();
        /*FILE* fp = fopen("../rawdata/votename.txt","r");
	fseek(fp,0,SEEK_END);
	long fs = ftell(fp);
	rewind(fp);
	...
	fs = ftell(fp);
	rewind(fp);
	char* votes = (char*)malloc(fs+1);
	fread(votes,1,fs,fp);
	fclose(fp);
	free(buf);
	double* majority = (double*)malloc(n*sizeof(double));
	...
	for (int i = 0; i < n; i++)
	{
		...
		//majority[i] = atof(votes);
		//votes += strlen(votes)+1;
		for (int y = 0; y < num_label; y++)
		{
			//if(sample->node[i]->label_type == Enum::KNOWN_L/ABEL && y != sample->node[i]->label){
			//	factor_graph->SetVariableStateFactor(i, y, 0);
			//}
			if (sample->node[i]->label_type == Enum::KNOWN_LABEL)
			{
				factor_graph->SetVariableStateFactor(i, y, y==sample->node[i]->label);
			}
			else
			{
				//double v = 1;
				...
				//factor_graph->SetVariableStateFactor(i, y, this->PyStateFactors[i*num_label+y]);
				//factor_graph->SetVariableStateFactor(i, y, y?majority[i]:1-majority[i]);:
			}
	...
	*/
	...
	for (int i = 0; i < n; i++)
	{
		...
		for (int y = 0; y < num_label; y++)
		{
			...
			//for (int t = 0; t < sample->node[i]->num_attrib; t++)
			//	v *= exp(p_lambda[sample->node[i]->attrib[t]] * sample->node[i]->value[t]);
			//factor_graph->SetVariableStateFactor(i, y, this->PyStateFactors[i*num_label+y]/*y?majority[i]:1-majority[i]*/);
			p_lambda += num_attrib_type;
		}
	}    
...
void CRFModel::SelfEvaluate()
{
	...
	for (int s = 0; s < ns; s++)
	{
		...
		//		for (int t = 0; t < sample->node[i]->num_attrib; t++)
		//			v *= exp(p_lambda[sample->node[i]->attrib[t]] * sample->node[i]->value[t]);
			//	factor_graph->SetVariableStateFactor(i, y, this->PyStateFactors[i*num_label+y]/*y?majority2[i]:1-majority2[i]*/);
...
void CRFModel::Evalute()
{
	...
	for (int s = 0; s < ns; s++)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			{
				//double v = 1.0;
				//fo/r (int t = 0; t < sample->node[i]->num_attrib; t++)
				//	v *= exp(p_lambda[sample->node[i]->attrib[t]] * sample->node[i]->value[t]);
				//factor_graph->SetVariableStateFactor(i, y, this->PyStateFactors[i*num_label+y]);
